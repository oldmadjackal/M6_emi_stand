...
#define  T    this->traces[i]
         ...
         if(T.missile==NULL)  continue ;              /* Если ЗУР не нашлась среди объектов :-( */
            ...
            r  =sqrt((T.target->x_base-this->x_base)*(T.target->x_base-this->x_base)+
                     (T.target->y_base-this->y_base)*(T.target->y_base-this->y_base)+
                     (T.target->z_base-this->z_base)*(T.target->z_base-this->z_base) ) ;
            r_m=sqrt((T.missile->x_base-this->x_base)*(T.missile->x_base-this->x_base)+
                     (T.missile->y_base-this->y_base)*(T.missile->y_base-this->y_base)+
                     (T.missile->z_base-this->z_base)*(T.missile->z_base-this->z_base) ) ;
...
/*- - - - - - - - - - - - - - - - - - - -  Выделение воздушных целей */
         if(OBJECTS[i]->y_base<=0)  continue ;                      /* Если это - НЕ воздушная цель... */

        if(this->ballistic_target==1) 
         if(OBJECTS[i]->y_velocity>=0)  continue ;                  /* Если это - НЕ баллистическая цель... */
/*- - - - - - - - - - - - - - - -  Отсечение целей по зоне поражения */
         if(OBJECTS[i]->y_base>this->h_firing_max)  continue ;      /* Если цель слишком высоко... */

            r=sqrt((OBJECTS[i]->x_base-this->x_base)*(OBJECTS[i]->x_base-this->x_base)+
                   (OBJECTS[i]->z_base-this->z_base)*(OBJECTS[i]->z_base-this->z_base) ) ;
...
/*- - - - - - - - - - - - Расчет углов наведения в упрежденную точку */
          missile=this->missile_tmpl ;
               ...
               dx=target->x_base-this->x_base ;
               dy=target->y_base-this->y_base ;
               dz=target->z_base-this->z_base ;

                a=target ->x_velocity*target ->x_velocity
                 +target ->y_velocity*target ->y_velocity
                 +target ->z_velocity*target ->z_velocity
                 -missile->x_velocity*missile->x_velocity
                 -missile->y_velocity*missile->y_velocity
                 -missile->z_velocity*missile->z_velocity ;
                b=2.*(target->x_velocity*dx+target->y_velocity*dy+target->z_velocity*dz) ;
                c=dx*dx+dy*dy+dz*dz ;
             ...
             tx_2=(-b+sqrt(b*b-4.*a*c))/(2.*a) ;
              ...
              dx+=target->x_velocity*tx_1 ;
              dy+=target->y_velocity*tx_1 ;
              dz+=target->z_velocity*tx_1 ;
             ...
             sprintf(command, "EXEC %s COPY %s;"
                              ...
                              "START %s;",
                               this->missile_type, this->traces[j].missile_name,
                                                   ...
                                                   this->traces[j].missile_name, this->x_base, this->y_base, this->z_base,
             ...
             sprintf(command, "EXEC %s COPY %s;"
                              ...
                              "START %s;",
                               this->missile_type, this->firing_channel->missile_name,
                                                   ...
                                                   this->firing_channel->missile_name, this->x_base, this->y_base, this->z_base,
