...
/*- - - - - - - - - - - - - - - - - - - - - -  Программа LONE_HUNTER */ 
     if(!stricmp(name, "LONE_HUNTER"  )) {
...
#undef   P
     ...
     if(!stricmp(PROGRAM_NAME, "LONE_HUNTER"  )) {
...
#define  PROGRAM_NAME  ((RSS_Unit_Program_Embeded *)embeded)->program_name
     ...
     else if(!stricmp(PROGRAM_NAME, "LONE_HUNTER"  ))  status=EmbededLoneHunter  (t1, t2, callback, cb_size) ;
     else                                        {
               SEND_ERROR("Section PROGRAM: Неизвестная встроенная программа") ;
                            return(-1) ;
                                                 }
...
/*--------------------------------------------- Выход на боевой курс */
   ...
   if(P->stage==3) {
    ...
    if(EventObject->y_base >= P->drop_height) {
                                                 P->stage   = 4 ;
                                                 P->e_direct= 0 ;
                                                 P->fired[0]= 0 ;
