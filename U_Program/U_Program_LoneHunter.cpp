...
/*                                                                  */
/*               Встроенная программа Lone Hunter                   */
/*                                                                  */
/*  Этапы программы:                                                */
...
/*	     Считывание настроек встроенной программы               */
  ...
  void *RSS_Module_Program::ReadLoneHunter(char *path, char *error)

{
                                          FILE *file ;
                                          void *data ;
           RSS_Unit_Program_Embeded_LoneHunter *data_LoneHunter ;
                                          char  text[256] ;
                                          char *key ;
                                          char *value ;
                                          char *end ;
...
/*- - - - - - - - - - - - - - - - - - - - - Обработка вида программы */ 
         if(data==NULL) {
              ...
              if(stricmp(value, "LONE_HUNTER"))  break ;

                 data_LoneHunter=(RSS_Unit_Program_Embeded_LoneHunter *)
                                         calloc(1, sizeof(*data_LoneHunter)) ;
                 data             =data_LoneHunter ;

                     strcpy(data_LoneHunter->program_name, value) ;
...
/*               Встроенная программа Lone Hunter                   */
     ...
     int  RSS_Unit_Program::EmbededLoneHunter(double t1, double t2, char *callback, int cb_size)
{
                 int  status ;
                char *g_type ;
...
/*----------------------------------- Выход в начальную точку поиска */
   ...
   if(P->stage==0) {
            ...
            if(P->route_cnt<2) {
                       SEND_ERROR("EmbededLoneHunter - Не задан маршрут патрулирования (минимум 2 точки)") ;
                                      return(-1) ;
                               }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - Поиск цели */
     if(P->stage_1_radar[0]!=0) {
       ...
       if(status) {
                        sprintf(text, "EmbededLoneHunter - %s", error) ;
                     SEND_ERROR(text) ;
                          return(-1) ;
                  }
...
/*------------------------ Неизвестная ветвь программного управления */
                       ...
                       SEND_ERROR("EmbededloneHunter - Unknown stage") ;
                                      return(-1) ;
                   }
/*-------------------------------------------------------------------*/
