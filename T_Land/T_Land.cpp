...
#define  _SECTION_FULL_NAME   "LAND"
#define  _SECTION_SHRT_NAME   "LAND"
...
/*		    	Программный модуль                          */
     ...
     static   RSS_Module_Land  ProgramModule ;
...

{
 ...
 T_LAND_API RSS_Kernel *GetEntry(void)
...
/********************************************************************/
/**							           **/
/**	       ОПИСАНИЕ КЛАССА ЗАДАЧИ "СОЗДАНИЕ ЛАНДШАФТА"         **/
/**							           **/
/********************************************************************/
/********************************************************************/
...
/*		     Общие члены класса             		    */
      ...
      struct RSS_Module_Land_instr *RSS_Module_Land::mInstrList       =NULL ;

                               OBJ  RSS_Module_Land::mObjects[_OBJECTS_MAX] ;
                               int  RSS_Module_Land::mObjects_cnt     =  0 ;

                             FRAME *RSS_Module_Land::mSpawns[_SPAWNS_MAX] ;
                               int  RSS_Module_Land::mSpawns_cnt      =  0 ;

                             FRAME *RSS_Module_Land::mScenario        =NULL ;
                               int  RSS_Module_Land::mScenario_cnt    =  0 ;

                               VAR *RSS_Module_Land::mVariables       =NULL ;
                               int  RSS_Module_Land::mVariables_cnt   =  0 ;
                              HWND  RSS_Module_Land::mVariables_Window=NULL ;

                               int  RSS_Module_Land::mHit_cnt ;

                              HWND  RSS_Module_Land::mMapWindow ;
                               int  RSS_Module_Land::mMapRegime ;
                            double  RSS_Module_Land::mMapXmin ;
                            double  RSS_Module_Land::mMapXmax ;
                            double  RSS_Module_Land::mMapZmin ;
                            double  RSS_Module_Land::mMapZmax ;
                             COLOR  RSS_Module_Land::mMapColors[_OBJECTS_MAX] ;
                               int  RSS_Module_Land::mMapColors_cnt ;
                             TRACE  RSS_Module_Land::mMapTraces[_OBJECTS_MAX] ;
                               int  RSS_Module_Land::mMapTraces_cnt ;
...
/*		       Конструктор класса			    */
     ...
     RSS_Module_Land::RSS_Module_Land(void)
...
/*---------------------------------------------------- Инициализация */
         ...
         identification="Land" ;
             ...
             mInstrList=RSS_Module_Land_InstrList ;

                Context=new RSS_Transit_Land ;

/*----------------------------------- Регистрация класса элемент Map */

          mMapRegime=_MAP_KEEP_RANGE ;

	Map_wnd.lpszClassName="Task_Land_Map_class" ;
	Map_wnd.hInstance    = GetModuleHandle(NULL) ;
	Map_wnd.lpfnWndProc  = Task_Land_Map_prc ;
...
/*		        Получить параметр       		    */
     ...
     int  RSS_Module_Land::vGetParameter(char *name, char *value)
...
/*-------------------------------------------------- Описание модуля */
    ...
    if(!stricmp(name, "$$MODULE_NAME")) {
         ...
         sprintf(value, "%-20.20s -  Создание ландшафта", identification) ;
                                        }
/*-------------------------------------------------------------------*/
...
/*		        Выполнить команду       		    */
  ...
  int  RSS_Module_Land::vExecuteCmd(const char *cmd)

{
  ...
  int  RSS_Module_Land::vExecuteCmd(const char *cmd, RSS_IFace *iface)
   ...
   if(end==NULL || end[0]==0) {
     if(!direct_command) {
        ...
        SendMessage(this->kernel_wnd, WM_USER,
                     (WPARAM)_USER_COMMAND_PREFIX, (LPARAM)"Task Land:") ;
        SendMessage(this->kernel_wnd, WM_USER,
...
/*--------------------------------------------- Выделение инструкции */
   ...
   for(i=0 ; mInstrList[i].name_full!=NULL ; i++)                   /* Ищем команду в списке */
     ...
     if(mInstrList[i].name_full==NULL) {                            /* Если такой команды нет... */
       ...
       if(status)  SEND_ERROR("Секция LAND: Неизвестная команда") ;
                                            return(-1) ;
                                       }
...
/*		        Считать данные из строки		    */
    ...
    void  RSS_Module_Land::vReadSave(std::string *data)

{

/*----------------------------------------------- Контроль заголовка */

   if(memicmp(data->c_str(), "#BEGIN MODULE LAND\n", 
                      strlen("#BEGIN MODULE LAND\n")) )  return ;
...
/*		        Записать данные в строку		    */
    ...
    void  RSS_Module_Land::vWriteSave(std::string *text)

{

/*----------------------------------------------- Заголовок описания */

     *text="#BEGIN MODULE LAND\n" ;
...
/*             Выполнить действие в контексте потока                */
    ...
    void  RSS_Module_Land::vChangeContext(void)
...
/*		      Реализация инструкции HELP                    */
  ...
  int  RSS_Module_Land::cHelp(char *cmd, RSS_IFace *iface)

{ 
    DialogBoxIndirect(GetModuleHandle(NULL),
			(LPCDLGTEMPLATE)Resource("IDD_HELP", RT_DIALOG),
			   GetActiveWindow(), Task_Land_Help_dialog) ;
...
/*       ADD <Имя объекта>                                          */
  ...
  int  RSS_Module_Land::cAdd(char *cmd, RSS_IFace *iface)
...
/*       CLEAR                                                      */
  ...
  int  RSS_Module_Land::cClear(char *cmd, RSS_IFace *iface)
...
#define   OBJECTS_CNT   RSS_Kernel::kernel->kernel_objects_cnt
       ...
       for(i=0 ; i<OBJECTS_CNT ; i++) {
         ...
         if(OBJECTS[i]->land_state==_ACTIVE_STATE) {

                                OBJECTS[i]->vPop() ;
                                OBJECTS[i]->vCalculateShow(0, 0) ;
                                                   }
         if(OBJECTS[i]->land_state== _SPAWN_STATE) {
...
/*		      Реализация инструкции LIST                    */
  ...
  int  RSS_Module_Land::cList(char *cmd, RSS_IFace *iface)

{ 
    DialogBoxIndirectParam(GetModuleHandle(NULL),
			    (LPCDLGTEMPLATE)Resource("IDD_LAND", RT_DIALOG),
			       GetActiveWindow(), Task_Land_View_dialog, (LPARAM)this) ;
...
/*    {T|DT}=<метка времени> EXEC  <Системная команда>              */
  ...
  int  RSS_Module_Land::cProgram(char *cmd, RSS_IFace *iface)
...
/*--------------------------------------------- Контроль имени файла */
    ...
    if(pars[0]==NULL) {                                             /* Если имя файла не задано... */
         ...
         if(       end       ==NULL ||                              /*   - добавляем типовое             */
            strchr(end, '/' )!=NULL ||
            strchr(end, '\\')!=NULL   )  strcat(path, ".land") ;
...
/*		      Реализация инструкции RUN                     */
  ...
  int  RSS_Module_Land::cRun(char *cmd, RSS_IFace *iface)
...
/*----------------------------------------------- Подготовка попытки */
      ...
      for(i=0 ; i<mObjects_cnt ; i++) {
            ...
            mObjects[i].object->vSpecial       ("LAND", this) ;
...
/*        MAP <Квадрант отображения>                                */
  ...
  int  RSS_Module_Land::cMap(char *cmd, RSS_IFace *iface)
...
/*  exec   -  Выполнить системную команду                           */
  ...
  int  RSS_Module_Land::iFrameExecute(FRAME *frame, double  t, int sync_use)
...
/*- - - - - - - - - - - - - - - - - - -  Добавление объекта в список */
     if(mObjects_cnt>=_OBJECTS_MAX) {
         ...
         if(object->land_state!=_SPAWN_STATE) {                   /* Если объект не создан в процессе боя... */
                   object->battle_state=_ACTIVE_STATE ;
                   object->vPush() ;
                                              }

                   object->vSpecial       ("LAND", this) ;
...
/*--------------------------------------------- Неизвестная операция */
                 ...
                 sprintf(text, "LAND - Неизвестная операция: %s", frame->action) ;
              SEND_ERROR(text) ;
                  return(_EXIT_FRAME) ;
                                        }
/*-------------------------------------------------------------------*/
...
/*	       Конструктор класса "ТРАНЗИТ КОНТЕКСТА"      	     */
     ...
     RSS_Transit_Land::RSS_Transit_Land(void)
...
/*	              Исполнение действия                           */
    ...
    int  RSS_Transit_Land::vExecute(void)
